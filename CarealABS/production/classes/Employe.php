<?php
	...
	*********************/
	...
	class Employe // Validé.
	{
		...
		public function simpleArrayByMonthPres($matricule, $month) // Format de $month YYYY-MM
		{
			$checkMonth = [];
			$monthOnly = substr($month, 5, 2); // On récupère juste le mois.
			$yearOnly = substr($month, 0, 4); // On récupère juste l'année.
			...
			$i = 0;
			while (checkdate($monthOnly, ($i + 1), $yearOnly)) { 
				$checkMonth[$i] = $this->getHour($matricule, $format.'-'.($i + 1))/8*100;
				$i++;
			}
			return $checkMonth;
		}
		public function simpleArrayByMonthAbs($matricule, $month) // Format de $month YYYY-MM
		{
			...
			$i = 0;
			while (checkdate($monthOnly, ($i + 1), $yearOnly)) { 
				$checkMonth[$i] = $this->simpleHeuresAbsJour($matricule, $format.'-'.($i + 1));
				$i++;
			}
			return $checkMonth;
		}
		public function simpleArrayByMonthTauxAbs($matricule,$month) // Format de $month YYYY-MM
		{
			$checkMonth = [];
			$monthOnly = substr($month, 5, 2); // On récupère juste le mois.
			$yearOnly = substr($month, 0, 4); // On récupère juste l'année.
			...
			$i = 0;
			while (checkdate($monthOnly, ($i + 1), $yearOnly)) { 
				$checkMonth[0][$i] = $this->dayRate($matricule, $format.'-'.($i + 1));
		...
		public function simpleArrayByYearNbHAbs($matricule, $year) // Format YYYY
		{
			$checkYear = [];
			$tab = $this->simpleArrayByYear($matricule, $year);
			for ($i = 0; $i < 12; $i++) { 
				$checkYear[$i] = $this->simpleHeuresAbsMois($matricule,$year.'-'.($i+1));
			}
			return $checkYear;
		}
		public function simpleArrayByYearTauxAbs($matricule, $year) // Format YYYY
		{
			$checkYear = [];
			$tab = $this->simpleArrayByYear($matricule, $year);
			for ($i = 0; $i < 12; $i++) { 
				$checkYear[$i] = $this->monthRate($matricule,$year.'-'.($i+1));
			}
			return $checkYear;
		}
		...
		public function simpleArrayBand($matricule, $begin, $end) // $begin et $end ont le fomat YYYY-MM-DD
		{
			...
			$day = $begin;
			for ($i = 0; $i < $taille; $i++) { 
				$checkBand[0][$i] = $this->simpleHeuresTravJour($matricule, $day) / 8 * 100;
				$checkBand[1][$i] = $day;
		...
		public function simpleArrayBandTauxAbs($matricule, $begin, $end) // $begin et $end ont le fomat YYYY-MM-DD
		{
			$taille = $this->nbDays($begin, $end);
			$checkBand = [];
			$day = $begin;
			for ($i = 0; $i < $taille; $i++) { 
				$checkBand[0][$i] = $this->dayRate($matricule,$day);
				$checkBand[1][$i] = $day;
				$day = $this->nextDay($day);
			}
			return $checkBand;
		...
		public function simpleArrayBandAbs($matricule, $begin, $end) // $begin et $end ont le fomat YYYY-MM-DD
		{
			...
			$day = $begin;
			for ($i = 0; $i < $taille; $i++) { 
				$checkBand[0][$i] = $this->simpleHeuresAbsJour($matricule,$day);
				$checkBand[1][$i] = $day;
				$day = $this->nextDay($day);
			}
			return $checkBand;
